...
public:
    ...
    void deserialize(AggregateDataPtr __restrict place, ReadBuffer & buf, std::optional<size_t> /* version */, Arena *) const override
    {
        ...
        /// Read incrementally: `add`/`serialize` do not cap the number of time steps, so neither does deserialize.
        /// A corrupted payload that claims an enormous size then fails when the buffer runs out, without a huge upfront allocation.
        state.k_per_step.clear();
        for (size_t t = 0; t != num_steps; ++t)
        {
            UInt64 k = 0;
        ...
        /// `num_steps` is now known to be real (we just read that many `k` values), so reserving is proportional to the payload.
        state.heaps.clear();
        state.heaps.reserve(state.k_per_step.size());
        for (size_t t = 0; t != num_steps; ++t)
        {
            size_t heap_size = 0;
            ...
            state.heaps.push_back(std::move(heap));
        }
