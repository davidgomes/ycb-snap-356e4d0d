...
type resourceEstimateFactor struct {
	...
	// segments when tiered eviction is disabled. External tables download,
	// decompress and deserialize row groups into Arrow buffers, so peak
	// transient memory = rawDataSize * factor. Defaults to 2.0 via
	// paramtable queryNode.externalCollection.rawDataFactor.
	// When tiered eviction is enabled, skip this extra Go-side margin:
	// the caching layer already reserves cell storage and transient loading
	// overhead for the same raw data.
...
func estimateLoadingResourceUsageOfSegment(schema *schemapb.CollectionSchema, loadInfo *querypb.SegmentLoadInfo, multiplyFactor resourceEstimateFactor) (usage *ResourceUsage, err error) {
	...
	// specific behaviors:
	//   1. Non-lazy path with tiered eviction disabled: apply
	//      externalRawDataFactor to cover the peak transient memory during
	//      download + decompress + Arrow deserialize (normal packed segments
	//      do not have this peak because their binlogs are already in Arrow
	//      IPC format). When tiered eviction is enabled, skip this extra
	//      Go-side margin — the caching layer already reserves cell storage
	//      and transient loading overhead for the same raw data.
	...
	if typeutil.IsExternalCollection(schema) && loadInfo.GetNumOfRows() > 0 {
		...
		if isExternalCollectionLazyLoad(schema) {
			...
			if factor := multiplyFactor.externalRawDataFactor; factor > 1.0 {
				// Non-lazy, no caching layer → add peak margin on top of
				// rawSize that PART 2 added.
